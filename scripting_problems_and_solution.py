...
#The find_item function uses binary search to recursively locate an item in the list, returning
#True if found, False otherwise. Something is missing from this function.
...
#so we need to place debugging lines to let us know each time that the list is cut in half, whether we're on the left or the right. Nothing needs to be 
...
#"Checking the left side", then determines that it's in the right half of the new list and prints "Checking the right side", before returning the 
...
#Here, linear_search and binary_search functions both return the number of steps that it took to either locate
#the key, or determine that it's not in the list. If the number of steps is the same for both methods
#(including the extra step for sorting in binary_search),
